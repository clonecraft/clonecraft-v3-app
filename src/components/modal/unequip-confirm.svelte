<script lang="ts">
  ...
  import { unequip, getPayment } from '@/blockchain/contracts/Equip'
  ...
  export let parentModalState: boolean
  ...
  beforeUpdate(async () => {
    if (modalState) {
      const keyTokenAmount = baseKeyTokenAmount * assetData.length
    ...
    <div class="box">
      ...
      <div class="button-wrap">
        ...
        {#if equipButtonActive}
          <div class="normal-button" on:click={omegaUnequip}>Unequip</div>
        {:else}
          <div class="unnormal-button">Unequip</div>
        {/if}
      </div>
    </div>
  </div>
