<script lang="ts">
  ...
  const getMyV3List = async (address: string) => {
    const res = await axios.get(`https://craft-api.clonesneverdie.com/v3/in-wallet/${address}`)
    return res.data
  }

  const getMyAssetList = async (address: string) => {
    const res = await axios.get(`https://craft-api.clonesneverdie.com/asset/in-wallet/${address}`)
    return res.data
  }
</script>
